...
const Navbar = ({ isLoggedIn = false, userEmail = null, onAuthChange = null }) => {
  const router = useRouter();
  const [showUserMenu, setShowUserMenu] = useState(false);
  const [authState, setAuthState] = useState({ isLoggedIn, userEmail });
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (res.ok && data.isAuthenticated) {
          setAuthState({ isLoggedIn: true, userEmail: data.user.email });
          if (onAuthChange) onAuthChange(true, data.user.email);
        } else {
    ...
    // Only check auth if not explicitly provided
    if (isLoggedIn === false && userEmail === null) {
      checkAuth();
    } else {
      setAuthState({ isLoggedIn, userEmail });
    }
  }, [isLoggedIn, userEmail, onAuthChange]);
              ...
              <div className="relative">
                ...
                >
                  <div className="w-8 h-8 bg-gradient-to-r from-green-400 to-blue-500 rounded-full flex items-center justify-center text-white font-bold">
                    {getUserInitial(authState.userEmail)}
                  </div>
